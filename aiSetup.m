function [ cpu_ships ] = aiSetup()
    code = spriteCode();
    % the "board" of ships is initially empty: 10 by 10 of zeros
    cpu_ships = code.water_sprite * ones(10,10);
    ...
    for ship_id = 1:5
        ...
        while ~ship_placed % continue to try and place the ship until successful
            ...
            if horizontal
                ...
                if sum(cpu_ships(row,col:(col+ship_length(ship_id)-1))) == code.water_sprite * ship_length(ship_id)
                    % if it is, fill that part of the board with the ships id
                    % number
                    cpu_ships = place_ship(row, col, ship_length(ship_id), 'hor', cpu_ships);
                ...
                if sum(cpu_ships(row:(row+ship_length(ship_id)-1),col)) == code.water_sprite * ship_length(ship_id)
                    cpu_ships = place_ship(row, col, ship_length(ship_id), 'ver', cpu_ships);
                    ship_placed = true;
                end
            end
        end
